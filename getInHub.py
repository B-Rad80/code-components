...
def getCoord(addr):  # takes plaintext address and returns coordinates
    ...
    # puts paramaters for url in dictionary, to be passed in next line
    payload = {'address': addr, 'key': key}
    mapsResponse = requests.get(
        'https://maps.googleapis.com/maps/api/geocode/json', params=payload)  # makes request using url
# parses json-formatted response from plaintext into dictionary
    mapsJson = json.loads(mapsResponse.text)
...
def getCT(lat, lng):  # takes latitude and longitude and returns census tract number
    payload = {'x': lng, 'y': lat, 'benchmark': "Public_AR_Census2010", 'vintage': 'Census2010_Census2010',
               'layers': '14', 'format': 'json'}  # puts paramaters for url in dictionary, to be passed in next line
    censusResponse = requests.get(
        'https://geocoding.geo.census.gov/geocoder/geographies/coordinates', params=payload)  # makes request using url
    ...
    # parses json-formatted response from plaintext into dictionary
    censusJson = json.loads(censusResponse.text)
    # 11-digit census tract numbers are formatted 'XXYYYZZZZZZ' XX are state code,
    # YYY are county code and ZZZZZZ are tract code
    # Pulls each code from json dictionary
    state = str(censusJson['result']['geographies']
                ['Census Blocks'][0]['STATE'])
    county = str(censusJson['result']['geographies']
                 ['Census Blocks'][0]['COUNTY'])
    tract = str(censusJson['result']['geographies']
                ['Census Blocks'][0]['TRACT'])
...
class location():
    ...
    # inputted the string for the address. May be left blank if location is set from coordinates
    addressString = ""
    ...
    def __init__(self, addr):  # constructor. Takes addr string as input. If addr == "", constructor will not set location, but will construct class
        ...
        if addr != "":  # only sets location if addr isn't blank
            # Gets coordinates from google maps api, using plaintext address as input
            locInfo = getCoord(addr)
            if locInfo['error']:
                self.error = True
                self.errorText = "Maps API ERROR: ' " + \
                    locInfo['errorText'] + " '"
                if not locInfo['errorFatal']:
                    self.faddress = locInfo['faddr']
            else:
                # sets lat, lng, and faddress values and sets locSet to true
                self.lat = str(locInfo['lat'])
                self.lng = str(locInfo['lng'])
                self.faddress = locInfo['faddr']
                self.locSet = True
            if not self.error:
                # pulls tract and county codes from census api
                codes = getCT(self.lat, self.lng)
                if codes['error']:
                    self.error = True
                    self.errorText = "Census API Error: ' " + \
                        codes['errorText'] + " '"
                else:
                    self.tractCode = codes['tract']
                    self.countyCode = codes['county']
            if not self.error:
                self.designationInfo = designation.getDesignation(
                    self.countyCode, self.tractCode)  # pulls designation from database
                # sets overall designation to true or false
                self.designated = self.designationInfo['county'][
                    'currYearDes'] or self.designationInfo['tract']['currYearDes']
